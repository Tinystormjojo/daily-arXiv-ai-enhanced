...
### [1] [Automated Essay Scoring and Language Certification: Assessing Generalizability, Agreement and Validity for French](https://arxiv.org/abs/2606.02009)
...
### [2] [Site4Drug: Predicting Drug-Binding Target Sites with an AI Agent](https://arxiv.org/abs/2606.01816)
*Taehan Kim,Sarrah Rose Mikhail Leung,Bharat Mekala,Jeongbin Park*

Main category: cs.LG

TL;DR: This paper introduces Site4Drug, an AI agent for predicting drug-binding target sites on proteins, which can output ranked targetable regions with associated constraints, evidence summaries, risk flags, and traceable decision logs, and can also recommend drug binding modalities without requiring users to specify the modality upfront.


<details>
  <summary>Details</summary>
Motivation: Selecting targetable intervention sites on proteins, especially membrane proteins where accessibility, topology, and post-translational modifications constrain actionable regions, is a more ambiguous and failure-prone bottleneck than selecting binding molecules, so there is a need for better methods to identify drug-binding target sites.

Method: A modality-aware AI agent named Site4Drug is developed, which integrates multiple sources of evidence including topology, hydropathy, post-translational modification propensity, disulfides, domain context, and sequence for site discovery. It can automatically recommend binding modalities (antibody/peptide-like vs small-molecule) and consistently applies the evidence across different modalities including small-molecule pocket discovery to avoid selecting biologically occluded sites.

Result: Site4Drug can output a ranked list of targetable regions with explicit constraints, evidence summaries, risk flags, and a traceable decision log, and avoids selecting chemically plausible but biologically non-actionable occluded sites.

Conclusion: Site4Drug is a useful AI agent that can effectively identify drug-binding target sites and recommend appropriate drug binding modalities, addressing the bottleneck of target site selection in drug discovery.
...
### [3] [Time-Aware Diffusion based on Preference Disentanglement for Generative Recommendation](https://arxiv.org/abs/2606.01670)
*Bangguo Zhu,Peng Huo,Yuanbo Zhao,Zhicheng Du,Jun Yin,Senzhang Wang*

Main category: cs.IR

TL;DR: 本文提出了一种用于生成式推荐的时间感知扩散模型TDPM，通过将用户偏好解纠缠为长期周期偏好和近期点偏好，解决了现有扩散基生成式推荐对所有历史交互均匀应用扩散过程、未考虑用户偏好时间动态性的问题，在公开数据集上取得了显著优于SOTA基线的效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的生成式推荐方法对用户所有历史交互项目均匀应用扩散过程，忽略了用户偏好受多方面时间演化因素影响、在时间维度上呈现非平稳分布的特性，存在致命缺陷，因此需要针对该缺陷改进现有框架。

Method: 提出新型生成式推荐框架TDPM：1）在语义索引（SID）tokens上设计时间感知扩散过程；2）将用户偏好解纠缠为长期稳定的周期偏好和近期事件触发的点偏好；3）显式将时间演化的用户偏好影响整合进扩散过程。

Result: 在三个真实公开数据集上的实验表明，TDPM显著优于现有最优基线模型，在HR@20和NDCG@20指标上平均提升最高分别达29.21%和25.45%，消融实验验证了时间感知token扩散在扩散基生成式推荐中的必要性。

Conclusion: 将时间感知用户偏好整合到扩散过程、对偏好进行解纠缠可以有效提升扩散基生成式推荐的性能，所提TDPM框架显著优于现有SOTA方法。
...
### [4] [Breaking the Information Silo: Semantic Personas for Cross-Domain Recommendation](https://arxiv.org/abs/2606.01783)
*Jonathan Mayo,Moshe Unger,Konstantin Bauman*

Main category: cs.IR

TL;DR: 本文提出了SPHERE方法，针对无共享用户或物品的严格不相交跨域推荐任务，利用大语言模型生成语义用户 persona 实现知识迁移，提升推荐效果，得出跨域迁移效果的关键影响因素结论。


<details>
  <summary>Details</summary>
Motivation: 现有跨域推荐方法大多依赖共享用户、共享物品或结构相似交互图，这类假设在独立平台之间通常不成立，且数字平台多为孤立信息孤岛，无法构建跨域全面用户表示。

Method: 提出SPHERE框架，不通过身份或图结构对齐域，而是利用大语言模型归纳共享行为词汇表，为用户生成结构化语义persona，检索源域中行为相似社区得到社区源persona，再通过双塔结构和动态融合门将语义信号与协同信号融合，可增强标准推荐骨干模型。

Result: 在Amazon Books、Goodreads和Steam三个数据集的全排序评估中，相比NCF、SVD++和LightGCN基线实现了 consistent 性能提升；发现跨域迁移效果不单单由域间语义 proximity 决定，关键取决于目标域的结构密度和原生预测强度。

Conclusion: 将跨域个性化重定义为基于行为的语义对齐，为打破信息孤岛提供了可解释、模块化的实用机制，能在无共享用户/物品的跨域场景有效提升推荐性能。
