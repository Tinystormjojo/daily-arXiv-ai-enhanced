...
### [1] [Style, Not Self: Surface Cues Explain Zero-Shot Code Attribution by Large Language Models](https://arxiv.org/abs/2609.30048)
*Ehsan Barkhordar,Surendrabikram Thapa*

Main category: cs.CL

TL;DR: This paper investigates whether large language models (LLMs) can correctly attribute zero-shot code authorship (including self-recognition), finding that attribution performance mostly relies on surface style cues rather than intrinsic recognition of model-generated code, and that removing these surface cues eliminates most attribution ability.


<details>
  <summary>Details</summary>
Motivation: There is a concern that if LLMs can recognize code they generated, they may favor their own output when acting as judges, leading to collusion when models monitor each other. This work empirically tests zero-shot code attribution ability of current commercial LLMs to verify this capability and what drives it.

Method: The authors conducted experiments with multiple commercial LLMs: 5 LLMs generated solutions for MBPP, HumanEval, and DS-1000, 7 more generated solutions for MBPP, then models acted as evaluators across four different code attribution tasks. They also tested a rule-based normalization approach that strips surface cues (docstrings, comments, type hints, local names) to see if attribution performance changes, while preserving code functionality.

Result: 1. In single-solution self-authorship judgment, balanced accuracy only ranges 49-58% (near chance) across all tested combinations, with raw accuracy mostly reflecting how often models claim authorship. 2. Pairwise self-selection accuracy is highly correlated (r=0.93) with solution length, a surface cue. 3. Attribution to a named model works for some pairs but is inverted for others. 4. After removing surface cues via normalization, 10/12 attribution results drop to chance level, remaining results still rely on leftover length differences, and Claude Haiku's observed self-preference disappears, though a trained classifier can still separate most normalized pairs.

Conclusion: Zero-shot code attribution by LLMs is mostly explained by surface style cues rather than inherent ability to recognize self-generated code. The authors recommend reporting balanced accuracy, including heuristic baselines, and checking label consistency in future related work.
...
### [2] [Benchmarking and Domain Adaptation of Automatic Speech Recognition (ASR) for Adolescent Health Communication in Ghanaian Languages](https://arxiv.org/abs/2609.29798)
*Stephen E. Moore,Akwasi Asare,Mich-Seth Owusu,Paul Azunre,Joel Budu,Lawrence A. Adu-Gyamfi*

Main category: cs.CL

TL;DR: 本文对加纳三种语言（Twi、Dagbani、Ewe）用于青少年健康沟通的自动语音识别（ASR）系统进行了基准测试、领域适配，并在实际部署应用中进行验证，发现主要约束是领域内标注数据而非模型能力或计算资源。


<details>
  <summary>Details</summary>
Motivation: 目前针对加纳本地语言的青少年健康沟通场景的自动语音识别系统缺乏系统的基准测试和适配，难以支持实际的公共卫生应用，因此需要填补这一空白。

Method: 研究分为三个阶段：首先在通用圣经语料和青少年性与生殖健康（ASRH）领域数据集上，对5种ASR系统（3种特定语言Wav2Vec2模型和2种多模态大语言模型Gemma 3n、Gemma 4）进行基准测试，以字符错误率(CER)和词错误率(WER)为指标；然后基于基准结果选择了紧凑模型Qwen3-ASR-0.6B，使用大尺寸加纳圣经语料进行有监督领域适配，在领域内留存数据上评估；最后通过已部署的多语言语音应用KasaHealth和评估工具Senti-Check进行实际验证，由社区用户测试。

Result: 微调后所有语言的WER均下降，Ewe下降最为显著：WER从109.3%降至64.8%，CER从65.1%降至24.9%；实际部署测试中，KasaHealth获得了100%的聊天认可率，72%的良好/优秀翻译评分，92%的推荐意愿，同时也发现了限制实际使用的领域缺口。

Conclusion: 对于加纳这些低资源语言，约束ASR在青少年健康场景应用的主要因素是经过验证的领域内数据，而非模型能力或计算资源。
...
### [3] [SkinAgent AI: A Safety-Grounded Multimodal Agentic Framework for Non-Diagnostic Skincare Support](https://arxiv.org/abs/2609.29341)
*Muhammad Muhtasim Shahriar,Abdullah Mohammad Sayem,Tze Hui Liew,M. F. Mridha,Md. Mahiuddin*

Main category: cs.AI

TL;DR: 本文介绍并评估了一个面向消费者的非诊断护肤AI框架SkinAgent AI，这是一个基于安全约束的多模态智能体框架，验证了该框架在护肤辅助任务上的可行性，并指出其仍存在局限，需要进一步测试验证。


<details>
  <summary>Details</summary>
Motivation: 面向消费者的护肤AI需要在明确的证据和安全边界内协调视觉证据、产品信息、工具使用和用户交互动作，目前缺乏满足安全、可审计、边界约束要求的相关框架，因此提出SkinAgent AI来填补这一需求。

Method: SkinAgent AI结合了视觉问题路由与基于大语言模型的可审计编排架构，包含针对痤疮、毛孔、皱纹的路由模块、基于照片的肤质估计、计数驱动的有序痤疮严重程度辅助、类型化工具、基于数据库的推荐与动作功能、确定性安全隐私与证据检查、状态变更动作前的用户批准以及结构化追踪重放机制；分别对视觉模型性能和系统级智能体行为进行了评估测试。

Result: 皮肤状况路由模型准确率达到99.84% ± 0.07%，肤质估计准确率88.85%，计数驱动痤疮严重程度辅助准确率84.59%，二次加权kappa为0.9076；在240个案例的系统基准测试中，意图准确率80.00%，精确工具集匹配率62.92%，严格任务完成率47.08%；安全隐私测试中未观察到违规或跨用户信息泄露事件，但仍存在工具选择错误、产品属性 grounding 不完整、故障回退不可靠的问题。

Conclusion: 研究验证了有边界、基于数据库、可追踪的智能体编排用于非诊断护肤辅助的可行性，但该框架目前不具备临床适用性，不存在外部泛化性、正式隐私保证和普适安全性，仍需要独立验证、专家评估、鲁棒性公平性测试以及真实场景的前瞻性评估。
...
### [4] [Learning Better Reasoning for Generative Recommendation with Semantic IDs](https://arxiv.org/abs/2609.29973)
*Mengdan Zhu,Yufan Zhao,Sophie Di,Yao Zhao,Tao Di,Yulan Yan,Sridhar Iyer,Liang Zhao*

Main category: cs.IR

TL;DR: This paper proposes Evo-Rec, a three-stage reinforcement learning based framework to learn better reasoning for semantic ID (SID) based generative recommendation, which outperforms existing strong baselines on three Amazon Review benchmarks.


<details>
  <summary>Details</summary>
Motivation: Explicit reasoning before semantic-ID generation for generative recommendation can be inaccurate or uninformative, which may mislead subsequent item generation and degrade recommendation performance. There is a need for methods that can select effective reasoning traces and progressively improve reasoning from the model's own generations.

Method: The proposed Evo-Rec framework has three core stages: 1) align semantic IDs with their textual and behavioral contexts to enable the model to understand and generate item identifiers; 2) sample multiple candidate reasoning traces, retain those that improve ground-truth item prediction to get stronger reasoning initialization via supervised fine-tuning; 3) further optimize the reasoning policy via reinforcement learning with catalog-constrained item generation and ranking-aware recommendation feedback.

Result: Experiments on three Amazon Review benchmarks show that Evo-Rec consistently outperforms discriminative, generative, and existing reasoning-enhanced recommendation models across all evaluation metrics.

Conclusion: The proposed Evo-Rec framework is effective in learning better reasoning for semantic ID based generative recommendation, delivering improved recommendation performance over existing methods.
...
### [5] [Cross-Country Code-Mixing for Generative Recommendation](https://arxiv.org/abs/2609.28972)
*Yuan Gao,Hao Deng,Haibo Xing,Yi Xu,Lingyu Mu,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.IR

TL;DR: 本文针对跨国电商推荐中不同市场ID空间不相交、现有生成式推荐仅在参数层做知识迁移、数据层无知识共享的问题，提出了基于跨国家数据层注入监督的CMRec框架，通过代码混合合成混合国家序列提升数据稀疏国家的推荐效果，在真实数据集和线上AB测试中都取得了显著提升。


<details>
  <summary>Details</summary>
Motivation: 跨国电商推荐中不同市场的用户和物品ID空间相互独立，传统跨域方法缺少共享锚点；现有生成式推荐虽然通过共享 token 空间训练统一模型，但行为序列仍严格按国家区分，知识迁移只能发生在参数层，数据层面不存在知识迁移，因此需要解决这一缺陷。

Method: 提出CMRec跨国家生成式推荐框架：1. 从不同国家的多模态内容和行为共现中学习共享语义码本；2. 利用码本通过满足静态内容约束和动态价格、受众、流行度约束的token级替换合成混合国家序列；3. 引入上下文感知损失，根据混合样本在当前序列中的合理性重新加权。

Result: 在两个真实多国家数据集和大型电商平台的线上A/B测试中，CMRec显著提升了数据稀疏国家的推荐质量，同时保留了数据丰富国家的性能，使广告收入提升1.77%，订单量提升2.64%。

Conclusion: 在数据层通过双约束上下文感知的代码混合注入跨国家监督，能够有效提升跨国生成式推荐的效果，尤其对数据稀疏国家有明显增益，可落地到实际电商平台并带来业务指标提升。
...
### [6] [From Interests to Semantic IDs: Retrieval-Grounded Credit Assignment for Generative Recommendation](https://arxiv.org/abs/2609.29983)
*Mengdan Zhu,Yufan Zhao,Yao Zhao,Sophie Di,Tao Di,Yulan Yan,Sridhar Iyer,Liang Zhao*

Main category: cs.IR

TL;DR: 本文针对生成式推荐中基于语义ID（SIDs）的推理增强模型存在的信用分配稀疏和偏差问题，提出了基于检索 grounded 的信用分配方法，通过对生成的兴趣假设单独分配奖励，解决了信用分配缺口，在多个公开数据集上取得了推荐性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于语义ID的推理增强生成式推荐使用基于精确匹配SID奖励的组相对策略优化进行训练，在大目录下奖励非常稀疏，存在两种失败模式：组内所有生成都未命中目标时没有学习信号；相同SID奖励的生成即使推理轨迹不同也获得相同优势，奖励仅反映最终解码的SID而不评估推理过程，存在信用分配缺口。

Method: 提出检索接地查询归因方法：将生成轨迹结构化分为历史摘要、一组兴趣假设和最终SID；使用固定的检索器将每个兴趣假设作为目录查询执行，若查询在前K个结果中检索到目标则给予该假设奖励，将奖励定位到单个假设，在跨度级别分配信用，仅命中的假设获得正检索优势，检索通道不更新最终SID跨度。

Result: 在三个Amazon Reviews数据集上的实验显示，该方法持续提升了SID推荐性能；在Video Games数据集上的oracle分析表明，从生成兴趣中选择目标相关查询能够同时提升召回率和排序效果。

Conclusion: 所提出的检索接地信用分配方法解决了推理增强生成式推荐中的信用分配缺口，能够有效区分不同推理轨迹的质量，提升生成式推荐的性能。
...
### [7] [LSF-SR: Latent Semantic Fusion for Sequential Recommendation via Flow-based Conditional Variational Autoencoders](https://arxiv.org/abs/2609.29815)
*Shih-Hong Chen,Josh Jia-Ching Ying,Vincent S. Tseng*

Main category: cs.IR

TL;DR: This paper proposes LSF-SR, a novel sequential recommendation framework that fuses collaborative item ID embeddings and LLM-generated semantic signals via a flow-based conditional variational autoencoder, which outperforms state-of-the-art baselines on multiple public datasets.


<details>
  <summary>Details</summary>
Motivation: Existing sequential recommendation methods fail to effectively align collaborative signals with LLM-derived textual semantic knowledge, leading to item representations that do not leverage the complementary strengths of both types of signals and result in suboptimal recommendation performance.

Method: A novel framework LSF-SR is proposed, which uses a Conditional Variational Autoencoder (CVAE) integrated with Normalizing Flows (planar or radial flows) to fuse item ID collaborative embeddings and LLM-generated semantic signals. The core conditional fusion module learns a flexible latent space that clusters items with similar semantic profiles in the latent manifold.

Result: Extensive experiments on five public benchmark datasets show that LSF-SR consistently outperforms state-of-the-art baseline methods, achieving up to 12.98% improvement in Recall@20 and up to 14.13% improvement in NDCG@20.

Conclusion: The proposed latent semantic fusion framework LSF-SR effectively combines collaborative and semantic signals for sequential recommendation, delivering superior performance compared to existing state-of-the-art methods.
...
### [8] [X-Rec Technical Report](https://arxiv.org/abs/2609.29180)
...
TL;DR: 这是X-Rec生成式推荐检索框架的技术报告，针对现有推荐检索范式的缺陷提出基于流匹配的改进方案，在公开基准和TikTok线上部署都取得效果提升。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐检索的两种主流范式存在局限：U2I方法用确定性嵌入表示用户上下文，无法捕捉用户多样多模态兴趣；基于语义ID的自回归方法(SID-AR)虽然表达能力更强，但存在量化误差，且顺序解码推理吞吐量低，因此需要提出兼具效果和效率的生成式推荐检索方法。

Method: 提出X-Rec框架，通过流匹配直接在连续物品嵌入空间学习推荐分布，生成用于近似最近邻检索的嵌入触发，包含三个核心设计：1.锚点条件化，将生成分解为粗粒度语义区域选择和细粒度细化；2.采用黎曼流匹配，使生成轨迹对齐物品嵌入的超球面几何；3.设计后期交互扩散Transformer，将重复速度场估计限制在最后一个Transformer层，提升效率。

Result: 在流式基准测试中，X-Rec显著优于U2I基线，达到和SID-AR相当的检索质量，推理吞吐量比SID-AR高3.46倍；在TikTok线上部署后，垂直内容 engagement 提升4.1484%，整体engagement提升0.0111%。

Conclusion: X-Rec解决了现有生成式推荐检索范式的缺陷，兼顾了检索质量和推理效率，已成功部署到TikTok生产环境，带来了显著的业务效果提升。
...
### [9] [Advancing Model Research in AgentX: Long-Horizon Autonomy for Industrial Recommender Systems](https://arxiv.org/abs/2609.30001)
...
TL;DR: 本文介绍了面向工业推荐系统长周期自主研究的AgentX-Model框架，采用双智能体架构实现持续的模型研究流程，在工业生产评估和在线A/B测试中取得显著效果，并对研究分配调度进行了基准测试分析。


<details>
  <summary>Details</summary>
Motivation: 工业推荐系统的持续模型研究需要能够基于已有实验结果指导后续研究方向，现有流程缺乏能够连接提案开发和模型实验的自动化自主研究框架，无法有效支持长周期的迭代研究。

Method: 提出AgentX-Model框架，采用双智能体架构：Research Agent根据论文和已有实验结果生成独立评审的研究提案，Model Agent开展多轮实验并返回代码、指标和未解决问题；围绕Reproduce、Follow-up、Composition、Diagnose四类动作组织持续研究流程，其中前三者支撑常规研究，Diagnose收集修复问题所需证据（如业务反馈和在线评估发现的预测偏差）。

Result: 生产环境评估中，636个完成的模型修改实验里有560个AUC高于业务基线，部分实验AUC高于其谱系中所有可比前辈；五次最新在线A/B测试在不同业务场景取得增益：获客效率提升10-15%，目标区段广告消耗提升15-20%，观看时长提升0.3-0.8%，观看时长模型还减少了约10%的FLOPs和参数量；依赖感知历史回放基准测试显示，当智能体已经能分析选择具体候选时，更复杂的调度无法带来持续的效率提升。

Conclusion: AgentX-Model框架能够有效支持工业推荐系统的长周期自主模型研究，在实际生产中实现了稳定的性能增益和效率提升，复杂调度在已有智能体候选选择机制下无法进一步带来稳定效率提升。
