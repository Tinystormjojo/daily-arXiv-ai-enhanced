...
### [1] [GRPO for Financial Advice Generation: Outperforming Commercial LLMs under CATE Evaluation](https://arxiv.org/abs/2608.11787)
*Ofir Ben Shoham,Shrutendra Harsola,Vignesh Subrahmaniam,Shravan Mohan,Yakov Gazman,Oded Vainas*

Main category: cs.CL

TL;DR: 本文针对金融建议生成任务，采用Group Relative Policy Optimization（GRPO）强化学习方法微调开源大语言模型，使用基于LLM评判和安全门的奖励函数，并通过独立于评判器的CATE因果审计进行评估，最终得到的模型相比商业大语言模型实现了更高的利润提升、更低的下行风险，同时验证了因果审计对LLM评判的补充价值。


<details>
  <summary>Details</summary>
Motivation: 从业务记录生成可执行的金融建议需要模型整合数值推理、领域知识和判断能力，同时要避免有害建议，但该任务缺乏优质标注数据：历史决策不一定最优，高质量自由标注成本高昂；另外仅靠LLM-as-a-judge评估无法区分提升是真的业务价值还是过拟合到评判器，需要更可靠的评估方式。

Method: 1. 将金融建议生成建模为强化学习问题，使用GRPO算法微调开源大语言模型；2. 设计奖励函数：基于LLM-as-a-judge从多个二元维度评分建议质量，增加安全门实现伤害预防；3. 采用双稳健条件平均处理效应（CATE）估计器进行独立于LLM评判器的因果审计，验证真实业务价值。

Result: 在CATE因果审计下，训练后的模型估计毛利润提升约为最强商业基线的两倍（0.0228 vs 0.0104），同时拥有所有评估策略中最低的下行率和最小的负尾风险；两种评估方式对基线的排序不一致，未训练的基础模型在评判 rubric 中排最后，但在因果审计中排第二，说明因果审计能捕获LLM评判无法捕获的信号。

Conclusion: 带有金融领域奖励信号的GRPO可以生成比商业大语言模型更实用的业务推荐；在金融NLP中，独立于评判器的因果审计是对LLM-as-a-judge评估的有价值补充，而非单纯的验证。
...
### [2] [Locating and Controlling Implicit Personalization in Large Language Models](https://arxiv.org/abs/2608.11735)
*Yueru Yan,Siqi Wu,Thai Le*

Main category: cs.CL

TL;DR: 本文研究大语言模型中隐式个性化（即模型输出响应隐式人口统计提示发生偏移）的内部定位和控制方法，发现存在可追踪输出变化的局部内部激活信号，并验证了移除该信号可以有效抑制隐式个性化影响。


<details>
  <summary>Details</summary>
Motivation: 已有研究记录了大语言模型会对隐式人口统计提示做出输出偏移的隐式个性化行为，但这种行为变化和模型内部激活之间的关联尚不明确，缺乏对该现象的内部机制分析和有效控制方法。

Method: 针对五个大语言模型，构建匹配的提示会话和中性会话，分析内部激活信号与推荐输出变化的相关性，研究多提示共存时内部信号的组合模式，进一步通过移除特定提示对应的内部信号来验证控制隐式个性化影响的效果，并和prompt提示忽略人口统计信息的方法做对比。

Result: 1. 存在局部内部激活信号可以追踪推荐输出变化，相关系数最高可达r=0.87；2. 多个隐式提示共存时，其内部信号大致可组合，但输出变化不是简单叠加；3. 移除特定提示对应的内部信号比prompt提示更有效抑制其影响，且能基本保留模型通用基准性能；4. 选择性移除某一维度影响同时保留共存维度影响的能力高度依赖模型和属性类型。

Conclusion: 隐式个性化行为与可分析、可因果控制的内部激活信号相关，移除对应内部信号是抑制隐式个性化影响的有效方法，但选择性控制的效果存在模型和属性特异性。
...
### [3] [How to Spend Your Oracle Budget: Practical Guidance for Protein Structure Prediction Models](https://arxiv.org/abs/2608.12192)
*Aleksandra Kalisz,Jack Simons,Krisztina Sinkovics,Noam Ghenassia,Shikha Surana,Henry Moss,Paul Duckworth*

Main category: cs.LG

TL;DR: This paper benchmarks existing oracle guidance methods (including extending the O3 method to protein structure prediction) to provide practical guidance for allocating limited oracle budgets when correcting failures of protein structure prediction foundation models, and gives actionable recommendations based on experimental results.


<details>
  <summary>Details</summary>
Motivation: Protein structure prediction foundation models are unreliable for some targets; external biological oracles can correct failures but are expensive, so oracle budget is a key constraint. There is no systematic comparison of existing oracle budget allocation guidance methods to help practitioners select appropriate methods, creating a need for this benchmarking work.

Method: The authors benchmark existing methods (FK-steering, DPO, Best K-of-N sampling) alongside the recently proposed Optimisation Over Outputs (O3) method, which they extend for use in protein structure prediction models. They evaluate all methods on two protein targets (calmodulin 1CLL and E. coli aspartate transcarbamoylase 9EEH) across different oracle budgets.

Result: No single method consistently outperforms others across all budgets and oracle types. O3 is most effective at low oracle budgets, while FK-steering and DPO achieve better performance as the oracle budget increases.

Conclusion: This work provides the first systematic practical reference for oracle budget-aware guidance for protein structure prediction, and distills actionable recommendations for practitioners working under real-world oracle budget constraints.
...
TL;DR: This paper proposes HCGRec, a hint-conditioned generative recommendation framework for semantic-ID based generative recommenders, which solves the zero-reward optimization bottleneck in reward-based post-training and achieves significant performance improvements on sequential recommendation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Semantic-ID generative recommenders unify the generation interface for item IDs, interaction histories and item text, but they face a structured optimization bottleneck during reward-based post-training: if an early semantic token goes wrong, the model rarely reaches the ground-truth item in finite rollouts, leading to all-zero rewards in group-relative optimization and no useful learning signal.

Method: 1. HCGRec uses checkpoint rollouts to diagnose each training instance, and only provides a minimal target-prefix hint when the current generator cannot reach the correct item, turning zero-reward groups into informative comparisons. 2. It introduces hint-aware credit decomposition: supervised learning is used to preserve alignment for hinted tokens, while Group Relative Policy Optimization (GRPO) is applied to optimize the sampled unhinted suffix tokens.

Result: Experiments on sequential recommendation benchmarks show that HCGRec substantially outperforms supervised fine-tuning and vanilla reward-based post-training. It reduces the proportion of zero-advantage training samples from over 70% to below 20%.

Conclusion: HCGRec effectively recovers useful learning signal for hard training instances in semantic-ID generative recommendation, alleviates the zero-reward optimization bottleneck, and significantly improves model performance with lower percentage of useless zero-advantage samples.
...
### [5] [Learning from Online User Feedback for Shopping Agents](https://arxiv.org/abs/2608.11604)
*Haobo Zhang,Kelong Mao,Sulong Xu,Simiu Gu,Zhicheng Dou*

Main category: cs.AI

TL;DR: 本文提出了一个名为LOFA的框架，使基于大语言模型的购物代理能够在无需人工标注的情况下直接从真实在线用户交互日志中学习，实验证明该框架有效提升了购物代理的推荐质量、回复有用性和用户满意度一致性。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的购物代理已在电商平台落地，产生了大量有价值的用户交互日志可用于模型优化；现有方法主要依赖离线训练信号（如用户-物品交互、合成偏好数据），忽略了用户自然对话反馈中丰富的监督信息；同时现有在线反馈存在异质性、稀疏性和噪声问题，难以自动转化为可靠学习信号。

Method: 提出LOFA框架，无需人工标注即可从真实在线交互日志学习；该框架结合了基于可验证购买结果的强化学习，以及反馈感知的策略蒸馏，能够识别用户对话内指令并将其转化为稠密词级监督；两个互补目标同时捕捉协同行为模式和用户特定偏好。

Result: 在真实电商日志上的大量实验表明，相比强基线模型，LOFA在推荐质量、回复有用性和用户满意度一致性上均取得持续提升，验证了从真实在线用户反馈学习购物代理的有效性。

Conclusion: 利用在线用户自然对话反馈可以有效提升购物代理性能，所提出的LOFA框架能够自动处理异质稀疏噪声的在线反馈，无需人工标注即可稳定优化购物代理，效果优于现有基线方法。
