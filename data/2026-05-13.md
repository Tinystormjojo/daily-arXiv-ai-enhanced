...
TL;DR: 本文提出了一个用于评估大语言模型（LLM）在真实重症监护室（ICU）场景下长上下文理解能力的新基准RealICU，测试发现现有LLM在此任务上表现不佳，提出的结构化记忆代理ICU-Evo能够提升长时域推理能力，但仍未完全解决安全问题，该基准可为高风险临床决策支持AI的研究提供测试平台。


<details>
  <summary>Details</summary>
Motivation: ICU需要可靠的AI决策支持来辅助医生评估患者状态，现有ICU基准以临床医生历史行为作为 ground truth，但这些行为本身是在信息不完整、上下文有限的情况下做出的次优选择，无法准确评估AI系统的真实推理能力，因此需要更符合真实临床场景的评估基准。

Method: 1. 构建RealICU基准：由高级医师回顾完整患者轨迹后进行事后标注，设计了四个符合临床需求的任务（患者状态评估、急性问题识别、推荐动作识别、不安全风险红旗动作识别），按30分钟窗口分割患者轨迹，发布了两个数据集：包含94名患者930个窗口标注的RealICU-Gold，以及经医师验证的LLM标注器Oracle扩展得到的包含11862个窗口的RealICU-Scale。
2. 测试现有LLM（包括增强记忆的LLM）的表现，分析其失败模式。
3. 提出结构化记忆代理ICU-Evo，研究其对长时域推理能力的提升效果。

Result: 现有包括记忆增强在内的LLM在RealICU基准上表现很差，暴露了两个典型失败模式：临床推荐的召回-安全权衡，以及对患者早期解读的锚定偏差；提出的ICU-Evo结构化记忆代理能够改善长时域推理能力，但无法完全消除安全故障。

Conclusion: RealICU是一个符合临床实际的测试平台，可用于评测和改进高风险护理场景下AI序列决策支持能力，现有大语言模型在长上下文ICU数据推理上仍存在明显缺陷，有待进一步研究改进。
...
### [2] [Task-Aware Automated User Profile Generation for Recommendation Simulation Using Large Language Models](https://arxiv.org/abs/2605.13497)
*Xinye Wanyan,Chenglong Ma,Danula Hettiachchi,Ziqi Xu,Jeffrey Chan*

Main category: cs.IR

TL;DR: This paper proposes APG4RecSim, an automated task-aware user profile generation framework for LLM-based recommender system simulation using large language models, and validates its performance improvements over existing baselines on multiple benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: LLM-based agent simulation is important for evaluation of modern recommender systems, but existing work mostly focuses on improving memory and action modules of simulation frameworks while ignoring the pivotal profile generation component. Additionally, the lack of dedicated datasets for recommendation simulation leads to heavy reliance on manually crafted profiles, which limits the scalability and generalizability of simulation frameworks across different datasets.

Method: This work proposes the Automated Profile Generation Framework APG4RecSim for recommendation simulation, which constructs realistic, coherent, and robust task-aware user profiles with minimal supervision, leveraging large language models.

Result: Extensive experiments on three benchmark datasets show that APG4RecSim outperforms existing profile generation baselines on discrimination, ranking, and rating tasks: it improves ranking quality by up to 7% in nDCG@10 and reduces rating distribution divergence by 8% in JSD. The generated profiles are also resilient to popularity- and position-induced biases, and maintain stable performance across different datasets and different large language models.

Conclusion: APG4RecSim effectively addresses the limitations of existing profile generation methods for recommendation simulation, delivering better performance, stronger robustness, and better cross-dataset/cross-LLM generalizability with minimal supervision.
...
TL;DR: This paper proposes EcoGEO, an ecosystem-level Generative Engine Optimization (GEO) framework for web-enabled LLM search agents, and instantiates it with the TRACE trajectory-aware coordinated evidence ecosystem method, which outperforms page-level GEO baselines in open-ended product recommendation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Generative Engine Optimization (GEO) research only focuses on optimizing individual webpages, while agentic web search for LLM agents is a multi-step browsing process that requires synthesizing evidence across multiple documents. The influence of content on search outcomes depends not only on page content but also on how pages are organized, connected, and encountered along the agent's browsing trajectory, which existing work ignores.

Method: The paper formulates GEO as an environment-level influence problem (EcoGEO) for web-enabled LLM agents, and proposes TRACE, a trajectory-aware coordinated evidence ecosystem. Given a recommendation query and target product, TRACE builds a controlled evidence environment that coordinates a navigation entry page facing the agent with multiple heterogeneous support pages, which use shared terminology, internal links, and consistent product attributes to introduce, verify, and reinforce the target product across the browsing trajectory. Evaluation is conducted on the OPR-Bench open-ended product recommendation benchmark.

Result: Experimental results show that the proposed method consistently outperforms page-level GEO baselines in final target product recommendation. Trajectory-level metrics show that the method increases initial target-result crawls, target-specific follow-up searches, and internal-link crawls, confirming that performance gains come from shaping the agent's evidence-acquisition process rather than just adding more target-related content.

Conclusion: The findings support an ecosystem research paradigm for GEO, which studies web-enabled LLM search agents in the context of broader evidence environments that guide search, browsing, and answer synthesis.
...
TL;DR: 本文提出了一个面向昇腾NPUs的大规模生成式推荐加速训练系统TurboGR，通过三项核心创新解决了生成式推荐在昇腾NPUs上部署的系统瓶颈，在公开数据集上取得了高模型浮点利用率和近线性扩展性。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐是有前景的推荐范式，其推荐质量随模型容量和数据量提升而提升，但在昇腾NPUs上大规模部署存在根本性系统级挑战：缺少针对不规则（jagged）算子的高性能实现，且不规则稀疏原语和昇腾NPU面向稠密计算优化的架构不匹配。

Method: 提出了亲和昇腾的生成式推荐训练系统TurboGR，包含三个核心创新：1）昇腾亲和的不规则加速，通过融合算子消除填充冗余，动态负载平衡将设备间不均衡从47%降到2.4%；2）分布式通信优化，包括层次化稀疏并行、带收敛保证的半异步训练、细粒度流水线编排，可维持94%的NPU利用率；3）通过异步卸载、感知不规则的FP16量化、批内logit共享实现负采样优化，在不增加额外嵌入查找的情况下扩展有效负样本空间。

Result: 在KuaiRand-27K数据集上的评估显示，TurboGR支持训练最大0.2B参数的模型，达到54.71%的模型浮点利用率（MFU），且具有近线性扩展性（0.97）。

Conclusion: TurboGR有效解决了大规模生成式推荐在昇腾NPUs上训练的系统瓶颈，实现了高利用率和良好的扩展性，可支持亿级参数的生成式推荐模型训练。
...
### [5] [F-GRPO: Factorized Group-Relative Policy Optimization for Unified Candidate Generation and Ranking](https://arxiv.org/abs/2605.12995)
...
TL;DR: This paper proposes F-GRPO, a factorized Group-Relative Policy Optimization framework that unifies candidate generation and ranking for LLMs, addressing credit assignment issues in end-to-end optimization, and shows improved performance on sequential recommendation and multi-hop QA benchmarks over existing methods.


<details>
  <summary>Details</summary>
Motivation: End-to-end generative unification of candidate generation and ranking with LLMs suffers from a credit assignment gap: feedback is only given after the full ranked list is generated, so it cannot distinguish errors from bad candidate generation versus bad ranking, leading to unstable and sample-inefficient optimization. Decoupling the two stages solves this but introduces misalignment with downstream utility since ranking is limited by the input candidate set.

Method: The authors propose F-GRPO, a unified end-to-end optimized framework that factorizes the policy into candidate generation and ranking sub-components sharing a single LLM backbone. It uses two separate rewards: order-invariant coverage reward for generation and position-aware utility reward for ranking, and applies separate group-relative advantages for each phase to solve the credit assignment problem in a two-phase sequence-level objective.

Result: On sequential recommendation and multi-hop question answering benchmarks, F-GRPO outperforms vanilla GRPO, decoupled generation-ranking baselines, and supervised learning alternatives, and remains competitive with strong zero-shot rerankers without requiring inference-time architectural changes.

Conclusion: Factorized group-relative policy optimization enables effective end-to-end joint optimization of unified candidate generation and ranking in LLMs, improving performance over prior decoupled or un-factorized approaches while maintaining inference simplicity.
...
### [6] [Mix, Don't Tune: Bilingual Pre-Training Outperforms Hyperparameter Search in Data-Constrained Settings](https://arxiv.org/abs/2605.13225)
*Paul Jeha,Anastasiia Sedova,Louis Béthune,Skyler Seto,Jes Frellsen,Pierre Ablin,Natalie Schluter*

Main category: cs.LG

TL;DR: This paper compares two methods for improving low-resource language model pre-training under data constraints: aggressive hyperparameter tuning and mixing high-resource auxiliary language data. It concludes that mixing bilingual data outperforms hyperparameter search, with benefits scaling with model size.


<details>
  <summary>Details</summary>
Motivation: Most languages have limited pre-training data, leading to repeated training that degrades model generalization. Both hyperparameter tuning (via weight decay regularization) and mixing high-resource auxiliary language data are proposed remedies, but there is lack of systematic comparison between the two approaches across different model scales.

Method: The authors systematically compare hyperparameter tuning and bilingual (Arabic-English mixing across four model scales (150M to 1.43B parameters), using Arabic as the low-resource target language and English as the auxiliary high-resource language, conducting around 1000 pre-training runs to evaluate the two approaches.

Result: Three main findings: 1) Mixing auxiliary language data gives larger improvements than hyperparameter tuning on both validation loss and downstream task accuracy, and the performance gap increases with model size. 2) Mixing provides performance gains equivalent to 2-3x more unique target data for validation loss, and 2-13x for downstream accuracy, with gains scaling steeply with model size. 3) Target-language validation loss systematically underestimates the value of mixing, as it only captures the regularization effect of diversified training signal but not the new knowledge provided by auxiliary data.

Conclusion: Bilingual pre-training by mixing high-resource language data outperforms aggressive hyperparameter tuning in data-constrained settings. The practical recommendations are: mix in high-resource language data, prioritize tuning the mixing ratio over hyperparameter search, and transfer hyperparameters from small proxy models via μP.
