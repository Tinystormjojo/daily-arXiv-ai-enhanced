...
### [1] [URecJPQ: Memory-efficient Multimodal Recommendation Models through RecJPQ in Large-Scale Scenarios](https://arxiv.org/abs/2606.23291)
*Giuseppe Spillo,Zixuan Yi,Aleksandr Petrov,Cataldo Musto,Craig Macdonald,Iadh Ounis*

Main category: cs.IR

TL;DR: This paper proposes URecJPQ, a memory-efficient Joint Product Quantization method for large-scale multimodal recommendation, which drastically reduces trainable parameters and model size with only small average accuracy drops, and even achieves performance gains in some domains.


<details>
  <summary>Details</summary>
Motivation: Training state-of-the-art large-scale recommendation models requires huge memory for ID embeddings of massive users and items; this memory issue becomes even worse for multimodal recommendation, since additional multimodal features increase resource demands, which is not feasible when memory resources are limited.

Method: Proposes URecJPQ, a Joint Product Quantization method for large-scale multimodal top-k recommendation. Instead of using fully learned unique embeddings for each user/item, it represents each embedding as a concatenation of shared learned sub-embeddings, which reduces the total number of trainable parameters significantly.

Result: Experiments on three public datasets from different domains show that URecJPQ reduces checkpoint size by 86% to 98% and trainable parameters by 98% to 99% in large-scale scenarios, with only an average marginal accuracy decrease (8.5% for recall, 16% for NDCG), and delivers up to 85% performance improvement in the baby products domain.

Conclusion: URecJPQ can effectively reduce memory consumption of large-scale multimodal recommendation models with only minor average accuracy loss, and can even improve performance in some cases, making it suitable for large-scale scenarios.
...
### [2] [LLM-as-a-Judge for Reliable and Explainable Offline Evaluation in Top-K Recommendation](https://arxiv.org/abs/2606.22961)
*Yue Que,Junyi Zhou,Xiaokun Zhang,Haiming Jin,Qiao Xiang,Chen Ma*

Main category: cs.IR

TL;DR: 本文提出了一个用于Top-K推荐离线评估的LLM-as-a-Judge框架，解决了传统离线评估可靠性不足和缺乏可解释性的问题，通过实验验证了该框架在评估中具备良好的可靠性、可解释性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于留存用户行为的Top-K离线推荐评估存在两个核心缺陷：可靠性方面，将观测用户反馈作为真实偏好的代理，要求代理和推荐严格ID匹配，但反馈收集受到不完整和有偏的项目曝光影响，导致评估结果失真不可靠；可解释性方面，Top-K指标仅提供数值分数，无法给出有意义的支撑见解，让离线评估成为黑箱。因此需要新的评估方法解决这些问题。

Method: 提出LLM-as-a-Judge评估框架：1. 为提升可靠性，从用户文本行为构建语义偏好代理，在语义空间实现偏好和推荐的灵活匹配，不再依赖留存反馈；2. 为保证可解释性，LLM Judge采用先推理后评分的流程，生成相关性判断同时输出明确推理依据；3. 最后聚合个体得分得到全局Top-K指标，同时为每个偏好命中/未命中提供解释。

Result: 大量实验结果表明，所提出的LLM Judge框架在推荐离线评估中，具备出色的可靠性、可解释性和鲁棒性。

Conclusion: 所提LLM-as-a-Judge框架能够为Top-K推荐提供可靠且可解释的离线评估，解决了传统离线评估方法的核心缺陷。
...
### [3] [Who Owns the AI Recommendation? A Multi-Industry Empirical Map of Brand Category Ownership Across Large Language Models](https://arxiv.org/abs/2606.23057)
*Dmitrij Żatuchin*

Main category: cs.IR

TL;DR: This paper conducts a large-scale empirical study on brand ownership of AI-generated product/service recommendations across three major large language models (LLMs), proposes three metrics for measuring recommendation concentration and brand competition, and finds moderate concentration that contradicts the strong winner-takes-all narrative.


<details>
  <summary>Details</summary>
Motivation: LLMs increasingly mediate buyers' product/service discovery, making the competitive structure of AI-generated recommendations a key strategic concern for brands, but there is a lack of large-scale empirical answers to which brands LLMs recommend most in a category and how concentrated recommendation ownership is.

Method: The study collected 3,750 responses across 50 brands, 5 industries, 250 brand-free category queries on 3 LLMs (GPT-5.2, Google Gemini 3 Flash, Perplexity sonar-pro), with each query repeated 5 times under a stability protocol. Three exploratory metrics are proposed: Category Ownership Index (COI), Competitive Vacuum Index (CVI), and Displacement Score (DS) to measure brand share, competitive vacuums, and asymmetric brand substitution respectively.

Result: The results show that recommendation concentration is moderate with a mean Gini coefficient of 0.28, below the 0.60 power-law threshold. Competitive vacuums are rare (only 8.0% of queries). Cross-model agreement on the top-recommended brand is only 41.6%, meaning top positions are not consistent across models. Asymmetric displacement is industry-dependent, ranging from 0.4:1 in consulting to 4.3:1 with a mean of 2.4:1. Only 4.2% of topic clusters are outside original categories, indicating good category alignment.

Conclusion: The findings contradict the strong winner-takes-all narrative for AI recommendation within the studied scope. The three proposed metrics provide a reproducible procedure for competitive intelligence analysis that can be validated by future work.
...
### [4] [The Language Blind Spot: How Query Language and Brand Recognition Tier Shape AI-Constructed Brand Reputation Across Twelve European Languages](https://arxiv.org/abs/2606.23165)
*Dmitrij Żatuchin*

Main category: cs.IR

TL;DR: This paper investigates the "language blind spot" in AI-constructed brand reputation, measuring how query language and brand type affect LLM-generated reputation results across 12 European languages, and finds that English-only monitoring creates bias especially for local brands.


<details>
  <summary>Details</summary>
Motivation: Large language models increasingly mediate people's impressions of organizations, but most current AI brand reputation monitoring is done only in English, assuming English queries can yield representative results, which has not been empirically tested across multiple languages.

Method: Three grounded LLMs (GPT-5.4, Gemini 3.1 Pro, Perplexity Sonar Pro) were queried about 66 brands from 11 European markets in 12 languages from 4 language families, generating 35,640 responses. Multilingual BGE-M3 embeddings were used for cross-language comparison without translation, and statistical analysis was conducted to compare similarity, sentiment, recommendation share, and response stability across languages and brand types.

Result: 1. AI-constructed brand reputation is language-dependent: cross-language cosine similarity is 0.825, same-language family responses are more similar than cross-family, sentiment varies significantly by language with Uralic/Baltic languages being most positive and Germanic (including English) most critical. 2. Query language affects which brands are recommended far more than sentiment description: switching from English to a brand's home language increases recommendation share by 0.80 for local champions vs only 0.15 for global multinationals. 3. Response stability is affected more by model choice (eta^2=0.32) than language (eta^2=0.01).

Conclusion: English-only AI brand reputation monitoring leaves a measurable language blind spot, which is particularly concentrated in the underrepresentation of visibility of locally headquartered brands.
...
### [5] [Towards Fast Domain Adaptation and Fine-Grained User Simulation for Evaluating Conversational Recommender Systems](https://arxiv.org/abs/2606.22803)
*Yuanzi Li,Quanyu Dai,Xueyang Feng,Zihang Tian,Junhao Wang,Xu Chen,Zhenhua Dong,Huifeng Guo*

Main category: cs.IR

TL;DR: 本文针对对话推荐系统（CRS）评估中基于大语言模型的用户模拟器存在的领域适应性差、用户建模粗糙、评估有效性不足的问题，提出了自适应领域和自动提示调优的用户模拟器AdaptSim，实验证明它能生成真实对话，有效可靠评估CRS的能力和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 对话推荐系统的性能评估一直存在挑战，虽然基于大语言模型的用户模拟器已被证明有效，但仍存在三个关键缺陷：一是缺乏领域适应性，依赖固定提示和预定义动作空间阻碍了向新领域迁移；二是用户建模能力有限，无法精准复现细微语言风格和动态偏好；三是评估有效性不足，现有模拟器无法充分评估CRS的基础能力和系统鲁棒性，因此需要提出新的用户模拟器解决这些问题。

Method: 提出自适应领域和自动提示调优的用户模拟器AdaptSim：1. 利用自动提示生成和开放动作机制减少人工成本，提升跨领域灵活性；2. 在响应生成上，采用带有"思考-再响应"策略的可控文本生成，实现对语言风格的细粒度控制；3. 在CRS评估上，引入了新型基于广度优先搜索（BFS）的回合级 pairwise 比较框架实现全面评估。

Result: 在三个领域、四个大语言模型上开展的大量实验表明，AdaptSim能够生成真实符合用户特征的对话，能够对对话推荐系统的能力和鲁棒性实现高效可靠的评估。

Conclusion: AdaptSim解决了现有基于LLM的用户模拟器的三大缺陷，实现了快速领域适应和细粒度用户模拟，能够有效提升对话推荐系统评估的有效性和可靠性。
