<div id=toc></div>

# Table of Contents

- [cs.CL](#cs.CL) [Total: 1]
- [cs.IR](#cs.IR) [Total: 1]
- [cs.SE](#cs.SE) [Total: 1]
- [cs.SD](#cs.SD) [Total: 1]


<div id='cs.CL'></div>

# cs.CL [[Back]](#toc)

### [1] [Atomic Fact-Checking Increases Clinician Trust in Large Language Model Recommendations for Oncology Decision Support: A Randomized Controlled Trial](https://arxiv.org/abs/2605.03916)
*Lisa C. Adams,Linus Marx,Erik Thiele Orberg,Keno Bressem,Sebastian Ziegelmayer,Denise Bernhardt,Markus Graf,Marcus R. Makowski,Stephanie E. Combs,Florian Matthes,Jan C. Peeken*

Main category: cs.CL

TL;DR: 本文通过随机试验比较原子事实核查与传统可解释性方法对临床医生信任AI治疗建议的影响，发现原子事实核查能大幅提升医生信任度。


<details>
  <summary>Details</summary>
Motivation: 为明确不同AI可解释性方法对高风险临床决策中医生信任AI建议的影响，探究将AI建议分解为可验证声明并链接源指南文档的原子事实核查是否优于传统方法。

Method: 采用随机对照试验，招募356名临床医生生成7476份信任评级，对比原子事实核查与传统透明度机制对医生信任度的影响，使用Cohen's d衡量效应量。

Result: 原子事实核查对信任提升产生大效应（Cohen's d=0.94），使表达信任的医生比例从26.9%提升至66.5%；传统透明度机制相较于基线呈现剂量反应梯度改善（d=0.25-0.50），提升效果弱于原子事实核查。

Conclusion: 在高风险临床决策中，将AI建议分解为链接源指南的可独立验证声明，相较于传统可解释性方法能大幅提高临床医生的信任度。

Abstract: Question: Does atomic fact-checking, which decomposes AI treatment recommendations into individually verifiable claims linked to source guideline documents, increase clinician trust compared to traditional explainability approaches?
  Findings: In this randomized trial of 356 clinicians generating 7,476 trust ratings, atomic fact-checking produced a large effect on trust (Cohen's d = 0.94), increasing the proportion of clinicians expressing trust from 26.9% to 66.5%. Traditional transparency mechanisms showed a dose-response gradient of improvement over baseline (d = 0.25 to 0.50).
  Meaning: Decomposing AI recommendations into individually verifiable claims linked to source guidelines produces substantially higher clinician trust than traditional explainability approaches in high-stakes clinical decisions.

</details>


<div id='cs.IR'></div>

# cs.IR [[Back]](#toc)

### [2] [Beyond Similarity Search: A Unified Data Layer for Production RAG Systems](https://arxiv.org/abs/2605.03275)
*Venkata Krishna Prasanth Budigi,Siri Chandana Sirigiri*

Main category: cs.IR

TL;DR: 本文针对检索增强生成（RAG）系统在实际生产部署中性能不如原型的问题，分析了根源并提出基于PostgreSQL原生向量搜索的统一数据层方案，验证了其性能优势。


<details>
  <summary>Details</summary>
Motivation: RAG已成为大语言模型接入组织知识的标准架构，但生产部署中一直存在原型干净环境性能良好，而真实场景可靠性不足的差距，需要找出该差距的根源并解决。

Method: 首先定位出该差距的三个根本原因：数据过时、租户数据泄露、查询组合爆炸，发现都源于传统的分拆系统数据层；之后提出并评估构建在PostgreSQL上，结合原生向量搜索pgvector和HNSW索引的统一数据层，在5万文档上进行受控基准测试。

Result: 测试结果显示，该方案将日期过滤查询的延迟降低了92%，租户范围查询延迟降低74%，实现了零同步不一致，完全消除了跨租户数据泄露，同步代码减少了93%，另外文章还讨论了推荐的混合分层架构。

Conclusion: 传统分拆系统数据层是RAG生产可靠性差的根源，基于PostgreSQL原生向量搜索的统一数据层能有效解决现存问题，大幅提升RAG系统在真实生产环境中的性能和可靠性。

Abstract: Retrieval-Augmented Generation (RAG) systems have become the standard architecture for grounding large language models in organizational knowledge. Yet production deployments consistently expose a gap between clean prototype performance and real-world reliability. This paper identifies three root causes of that gap: data staleness, tenant data leakage, and query composition explosion. All three trace back to the conventional split-system data layer. We propose and evaluate a unified data layer built on PostgreSQL with native vector search (pgvector) and HNSW indexing. Controlled benchmarks on 50,000 documents show 92% latency reduction for date-filtered queries, 74% for tenant-scoped queries, zero synchronization inconsistency, and complete elimination of cross-tenant data leakage with 93% less synchronization code. We additionally discuss a recommended hybrid tier architecture

</details>


<div id='cs.SE'></div>

# cs.SE [[Back]](#toc)

### [3] [TeamUp: Semantic Project Matching and Team Formation for Learning at Scale](https://arxiv.org/abs/2605.03237)
*Dhruv Gulwani,Basem Suleiman,Aditya Joshi,Sonit Singh*

Main category: cs.SE

TL;DR: 针对大规模项目式学习中学生分配和认知多样化团队组建难题，提出基于嵌入的轻量级系统TeamUp，经实验验证其在匹配质量、难度对齐、团队多样性和效率成本上都有显著优势。


<details>
  <summary>Details</summary>
Motivation: 大规模项目式学习中，传统学生分配方法（手动表格、偏好调查）无法构建认知多样化协作团队，还会加剧公平性问题，表现为高成就学生自我选择优质项目、少数群体学生获得机会减少，因此需要新的解决方法。

Method: 提出TeamUp系统，利用预训练语言模型生成的语义嵌入将学生匹配到符合其技能水平的项目；采用混合排序算法结合余弦相似度和教学约束（难度对齐、领域偏好、需求平衡）生成个性化推荐；通过嵌入方差建模技能互补性，组建认知多样化团队保证团队能力分布均衡；通过虚拟实验对系统进行评估。

Result: 对比传统方法，实验结果显示：TeamUp匹配质量大幅提升（平均余弦相似度0.74 vs 0.43），难度对齐更好（83%学生分配在偏差一级内vs 34%），团队多样性更高（82%团队覆盖三个及以上技术领域vs 41%），同时推荐延迟低于一秒，每个学生运营成本低于0.1美元。

Conclusion: TeamUp是一种适用于大规模项目式课程的高效轻量级团队组建系统，能够有效提升匹配质量、难度对齐水平和团队认知多样性，同时成本低延迟小，可改善学习成果和公平性。

Abstract: Project-based learning improves student engagement and learning outcomes, yet allocating students to appropriately challenging projects while forming cognitively diverse teams remains difficult at scale. Traditional allocation methods (manual spreadsheets, preference surveys) can't construct the cognitively diverse teams that that collaborate cognitively. This mismatch perpetuates equity issues: high-performing students self-select visible projects while under-represented students face reduced access to opportunity. We propose TeamUp, a lightweight, embedding-based team-forming system designed to improve learning outcomes and equity in large-scale project-based courses. TeamUp uses semantic embeddings from pretrained language models to match students to projects aligned with their skill level. The system employs a hybrid ranking algorithm combining cosine similarity with pedagogical constraints (difficulty alignment, domain preferences, and demand balancing) to generate personalised and transparent recommendations. Beyond individual matching, TeamUp constructs cognitively diverse teams by modelling skill complementarity through embedding variance, ensuring teams possess well-distributed capabilities rather than homogeneous strengths. We evaluated TeamUp through a virtual experiment using 250 student profiles and 60 project descriptions. Results show: (1) substantially higher match quality (mean cosine similarity of 0.74 vs. 0.43); (2) better difficulty alignment (83% placed within one level vs. 34%); (3) more diverse teams (82% covering three or more technical areas vs. 41%); and (4) sub-second recommendation latency at operational costs under $0.10 per student.

</details>


<div id='cs.SD'></div>

# cs.SD [[Back]](#toc)

### [4] [APEX: Large-scale Multi-task Aesthetic-Informed Popularity Prediction for AI-Generated Music](https://arxiv.org/abs/2605.03395)
*Jaavid Aktar Husain,Dorien Herremans*

Main category: cs.SD

TL;DR: 本文针对AI生成音乐场景下的音乐热度预测问题，提出首个大规模多任务学习框架APEX，联合预测基于互动的热度指标和感知美学质量维度，验证了其在跨生成模型的偏好预测上的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 音乐热度预测受到大量关注，但AI生成音乐平台的兴起带来了新场景，该场景缺少传统的艺人声望、厂牌支持等标记，且美学质量这一关键因素尚未被探索，需要针对性的研究。

Method: 提出APEX框架，这是面向AI生成音乐的首个大规模多任务学习框架，基于来自Suno和Udio的超21.1万首（1万小时音频）数据训练，从自监督音乐理解模型MERT提取的冻结音频嵌入中，联合预测流播放量、点赞量这类基于互动的热度信号，以及五个感知美学质量维度。

Result: 在包含训练中未见过的11种生成音乐系统的 pairwise 人类偏好对决的Music Arena数据集做分布外评估，结果显示加入美学特征能够持续提升偏好预测精度，证明学习到的表示在不同生成架构间有很强的泛化能力，同时验证了美学质量和热度捕获了音乐互补信息，结合后更有效。

Conclusion: 在AI生成音乐热度预测中引入美学质量是有价值的，所提出的APEX框架能够有效联合学习热度和美学质量，在跨生成音乐架构的偏好预测任务上有优秀的泛化能力。

Abstract: Music popularity prediction has attracted growing research interest, with relevance to artists, platforms, and recommendation systems. However, the explosive rise of AI-generated music platforms has created an entirely new and largely unexplored landscape, where a surge of songs is produced and consumed daily without the traditional markers of artist reputation or label backing. Key, yet unexplored in this pursuit is aesthetic quality. We propose APEX, the first large-scale multi-task learning framework for AI-generated music, trained on over 211k songs (10k hours of audio) from Suno and Udio, that jointly predicts engagement-based popularity signals - streams and likes scores - alongside five perceptual aesthetic quality dimensions from frozen audio embeddings extracted from MERT, a self-supervised music understanding model. Aesthetic quality and popularity capture complementary aspects of music that together prove valuable: in an out-of-distribution evaluation on the Music Arena dataset, comprising pairwise human preference battles across eleven generative music systems unseen during training, including aesthetic features consistently improves preference prediction, demonstrating strong generalisation of the learned representations across generative architectures.

</details>
