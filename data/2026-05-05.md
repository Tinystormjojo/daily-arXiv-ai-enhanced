...
TL;DR: 本文针对检索增强生成（RAG）系统在生产部署中存在的原型性能和实际可靠性差距问题，分析了根源并提出基于PostgreSQL（pgvector）的统一数据层方案，通过基准测试验证了方案的优势。


<details>
  <summary>Details</summary>
Motivation: RAG已成为大语言模型接入组织知识的标准架构，但生产部署中始终存在干净原型性能与实际可靠性之间的差距，需要找出该差距的根本原因并提出解决方案。

Method: 首先定位差距的三个根本原因：数据陈旧、租户数据泄露、查询组合爆炸，发现三者都源于传统的分体式数据层；然后提出并评估了基于PostgreSQL、结合原生向量搜索pgvector和HNSW索引构建的统一数据层，在50000个文档上进行受控基准测试。

Result: 基准测试结果显示，该方案使日期过滤查询延迟降低92%，租户范围查询延迟降低74%，实现了零同步不一致，彻底消除跨租户数据泄露，同时同步代码减少了93%，文章还进一步讨论了推荐的混合分层架构。

Conclusion: 基于PostgreSQL带原生向量搜索的统一数据层能够有效解决传统分体式数据层带来的RAG系统生产部署可靠性问题，在延迟、数据一致性、安全性和代码量上都有显著提升。
