...
### [1] [Checkup2Action: A Multimodal Clinical Check-up Report Dataset for Patient-Oriented Action Card Generation](https://arxiv.org/abs/2605.11533)
*Sike Xiang,Shuang Chen,Kevin Qinghong Lin,Jialin Yu,Yijia Sun,Philip Torr,Amir Atapour-Abarghouei*

Main category: cs.CL

TL;DR: 本文介绍了一个名为Checkup2Action的多模态体检报告数据集与基准，用于面向患者的结构化行动卡片生成，提出了任务形式和评估协议，并通过实验对比不同大语言模型得到了相关结论，为该方向研究提供了评测基础。


<details>
  <summary>Details</summary>
Motivation: 普通民众难以解读体检报告中多模态异质信息并转化为具体后续行动，现有大语言模型在从多模态体检报告生成安全、优先级明确、面向患者的行动方面仍缺乏充分的基准评测。

Method: 构建包含2000份去标识化真实世界体检报告的Checkup2Action数据集，每个样本对应结构化行动卡片，包含临床问题、优先级、建议科室、随访时间窗、患者易懂解释和给临床医生的问题；将体检报告转行动生成建模为约束结构化生成任务，设计了覆盖问题覆盖率、准确率、优先级一致性、科室时间推荐准确率、行动复杂度、实用性、可读性和安全性合规性的评估协议，在通用和医学大语言模型上开展实验。

Result: 实验发现不同大语言模型在问题覆盖率、行动正确性、简洁性和安全对齐之间存在明显的权衡关系。

Conclusion: Checkup2Action为评估基于临床体检报告的面向患者推理提供了一个新的多模态基准，填补了该领域的 benchmark 空白。
...
### [2] [TwiSTAR:Think Fast, Think Slow, Then Act,Generative Recommendation with Adaptive Reasoning](https://arxiv.org/abs/2605.11553)
*Shiteng Cao,Kaian Jiang,Yunlong Gong,Zhiheng Li*

Main category: cs.IR

TL;DR: 本文提出了名为TwiSTAR的自适应推理生成推荐框架，针对基于语义ID的生成推荐固定推理策略存在的精度-延迟权衡问题，通过 planner 动态分配不同速度的推理工具，在提升推荐精度的同时降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 现有基于语义ID的生成推荐方法对所有用户历史统一采用固定推理策略：快速直接生成在难样本上精度不佳，始终使用慢的思维链推理会导致延迟过高、在简单样本上浪费计算资源，存在难以兼顾精度和效率的权衡问题。

Method: 提出TwiSTAR（Think Fast, Think Slow, Then Act）框架，学习为每个用户序列自适应分配推理工作量：为大语言模型配备三个互补工具：基于语义ID的快速检索器、轻量级候选排序器、生成显式理由后再推荐的慢推理模型；将物品间知识转换为自然语言解释，把协作常识注入慢推理模型；训练一个planner，先经监督预热再用智能体强化学习训练，动态决定调用哪个工具。

Result: 在三个公开数据集上的实验表明，该方法优于强基线模型，相比统一慢推理，在获得 consistent 精度提升的同时降低了推理延迟。

Conclusion: 自适应推理策略能够有效解决生成推荐中精度和推理延迟的权衡问题，TwiSTAR框架在保持推荐精度提升的同时有效降低了整体推理延迟，优于现有固定推理策略的生成推荐方法。
...
### [3] [Conditional Memory Enhanced Item Representation for Generative Recommendation](https://arxiv.org/abs/2605.11447)
*Ziwei Liu,Yejing Wang,Shengyu Zhou,Xinhang Li,Xiangyu Zhao*

Main category: cs.IR

TL;DR: 本文针对生成式推荐中当前项目表示构建存在的问题，提出了条件记忆增强的项目表示框架ComeIR，通过自适应评分、双级记忆存储和记忆复用恢复 token 粒度，实验证明了该方法的有效性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐的项目表示构建方法存在明显缺陷：直接合并SID token嵌入会放大量化和ID碰撞带来的信息损失，还会模糊SID代码关系；基于外部输入的方法虽然能增强项目语义，但无法可靠保留token级生成所需的SID结构化证据，进而导致了身份-结构保留冲突和输入-输出粒度不匹配两大问题，成为生成式推荐的性能瓶颈，需要解决。

Method: 提出条件记忆增强的项目表示框架ComeIR，具体包括三个核心部分：1）MM引导的token评分，自适应估计SID中每个代码的贡献；2）双级Engram记忆，捕捉项目内代码组成和项目间转移模式；3）记忆恢复预测头，在SID解码过程中复用存储的记忆，将SID token嵌入重建为项目感知输入，在解码时恢复token粒度。

Result: 大量实验结果表明ComeIR具备有效性和灵活性，并且实验进一步发现增大条件记忆可以带来可扩展的性能增益。

Conclusion: 所提出的ComeIR框架能够有效解决生成式推荐中项目表示构建的现有瓶颈，提升生成式推荐性能，且该方法具备可扩展性，增大条件记忆可进一步提升效果。
...
### [4] [Quality-Aware Collaborative Multi-Positive Contrastive Learning for Sequential Recommendation](https://arxiv.org/abs/2605.11707)
*Wei Wang*

Main category: cs.IR

TL;DR: 本文针对对比学习在序列推荐中存在的视图构造缺陷（语义漂移、多样性不足、未显式建模视图质量差异导致的假阳性问题），提出了一种质量感知的协同多正例对比学习方法QCMP-CL，通过可学习协同序列增强和质量感知自适应加权机制提升模型性能，实验验证其在多个真实数据集上优于现有基于对比学习的最优序列推荐基线模型。


<details>
  <summary>Details</summary>
Motivation: 对比学习在序列推荐中的效果依赖于兼具语义一致性和多样性的对比视图构造，现有方法存在以下缺陷：大多数基于对比学习的方法依赖启发式增强，容易丢失关键物品、破坏转移模式，导致语义漂移；少数可学习增强方法存在多样性不足的问题，仍需要启发式辅助，且很少显式自适应建模不同视图间的质量差异，加剧了假阳性问题。

Method: 1. 提出可学习的协同序列增强模块，基于同目标序列和相似序列两种互补协同上下文生成两个增强视图，在保留意图一致性的同时提升视图多样性；2. 设计与模型表示紧密集成的质量感知机制，根据增强操作的置信度估计每个视图的质量，分配自适应权重，让高置信度视图提供更多监督，低置信度视图提供更少监督。

Result: 在三个真实世界数据集上进行的大量实验表明，所提出的QCMP-CL模型性能优于所有现有基于对比学习的最优序列推荐基线模型。

Conclusion: 针对序列推荐中对比学习视图构造的现有缺陷，所提出的质量感知协同多正例对比学习框架能够有效提升视图质量，缓解语义漂移和假阳性问题，取得了优于SOTA基线的性能。
...
TL;DR: 本文针对多市场点击率预测中现有方法存在隐私泄露、无法处理市场异质性和跨市场ID空间不相交的问题，提出了联邦协作信号量化方法FedMM，在保证隐私的前提下提升了多市场CTR预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有多市场推荐大多采用集中式全局数据预训练加微调范式，会忽略市场数据隐私；传统联邦学习聚焦聚合参数得到全局模型，既没有考虑显著的市场异质性，且由于不同市场ID空间不相交，基于嵌入的聚合策略也无法生效，因此需要新的方法解决这些挑战。

Method: 提出FedMM方法，核心是利用离散码本机制实现隐私保护传输并对齐不相交的ID空间，采用分层码本结构捕捉跨市场共享模式和市场特定特征：每个市场部署带双层码本机制的残差量化变分自编码器（RQ-VAE）对协作嵌入进行量化，第一层是通过聚合更新的全局联邦码本捕捉通用共享协作模式，第二层维护本地码本学习市场特定语义，融合通用和特定协作信号的离散编码后输入下游CTR模型提升预测效果。

Result: 在基准数据集上的大量实验表明，FedMM在保证隐私的前提下，显著提升了多市场推荐（CTR预测）的性能。

Conclusion: FedMM可以有效解决多市场CTR预测中隐私保护、市场异质性和ID空间不相交的问题，在隐私保障下显著提升预测性能。
...
### [6] [AgentDisCo: Towards Disentanglement and Collaboration in Open-ended Deep Research Agents](https://arxiv.org/abs/2605.11732)
*Jiarui Jin,Zexuan Yan,Shijian Wang,Wenxiang Jiao,Yuan Lu*

Main category: cs.IR

TL;DR: 本文提出了用于开放式深度研究智能体的解耦协作架构AgentDisCo，在现有基准上取得了媲美领先闭源系统的性能，同时提出了新基准GALA，并展示了端到端个性化深度研究产品。


<details>
  <summary>Details</summary>
Motivation: 现有深度研究智能体方法将信息探索和利用过程合并到单个模块中，且现有基准无法充分反映真实世界用户需求，因此需要设计更合理的智能体架构并构建更贴合实际的评估基准。

Method: AgentDisCo将深度研究建模为信息探索和利用之间的对抗优化问题，解耦为两个智能体：critic智能体评估生成的大纲并优化搜索查询，generator智能体检索结果并修订大纲，迭代后生成的大纲交付给下游报告编写器生成研究报告；通过元优化框架支持人工设计和自动发现的设计策略，利用代码生成智能体探索智能体配置构建可复用策略库，实现无需大量人工干预的自优化；此外构建了从用户历史浏览行为挖掘潜在研究兴趣的新基准GALA，还开发了渲染智能体将研究报告转换为可视化海报。

Result: 在三个已有的深度研究基准上使用Gemini-2.5-Pro评估，AgentDisCo性能可媲美或超过领先的闭源系统；提出的新基准GALA更贴合真实用户需求，所展示的端到端产品AutoResearch Your Interest可基于用户浏览历史提供个性化深度研究推荐。

Conclusion: 解耦协作的架构AgentDisCo能够有效提升开放式深度研究智能体的性能，所提出的新基准GALA可以更好反映真实用户需求，该框架可以通过自优化不断改进，能够支撑端到端个性化深度研究产品。
...
### [7] [Why Users Go There: World Knowledge-Augmented Generative Next POI Recommendation](https://arxiv.org/abs/2605.11807)
*Qiuyu Ding,Heng-Da Xu,Wei Zhang,Dongyi Lv,Changda Xia,Feng Xiong,Mu Xu*

Main category: cs.AI

TL;DR: 本文提出了一种世界知识增强的大语言模型生成式下一个兴趣点（POI）推荐方法AWARE，解决现有LLM-based生成式POI推荐无法感知动态变化的真实世界出行影响因素的问题，在三个真实数据集上取得了相比基线最高12.4%的相对提升。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的生成式POI推荐模型，训练完成后编码的知识是固定的，无法感知影响用户出行决策的动态变化真实世界条件，比如本地活动、文化趋势这类信息，存在信息缺陷，需要引入动态的外部世界知识增强推荐效果。

Method: 提出AWARE（基于智能体的世界知识增强推荐框架）：利用LLM智能体生成和位置、时间感知的上下文描述，捕捉区域文化特征、季节趋势、和用户相关的当前事件；进一步将这些描述锚定到用户自身行为语境中，把外部世界知识和个性化时空行为模式结合，避免引入通用或噪声信息。

Result: 在三个真实世界数据集上进行的大量实验表明，AWARE consistently优于多个有竞争力的基线模型，取得了最高12.4%的相对性能提升。

Conclusion: 引入位置时间感知的动态世界知识并将其锚定到用户个性化行为语境，可以有效提升生成式下一个POI推荐的性能，AWARE方法相比现有基线模型获得了显著的效果提升。
...
### [8] [Intermediate Artifacts as First-Class Citizens: A Data Model for Durable Intermediate Artifacts in Agentic Systems](https://arxiv.org/abs/2605.12087)
*Josh Rosen,Seth Rosen*

Main category: cs.AI

TL;DR: This paper proposes treating intermediate artifacts in agentic AI systems as first-class citizens, presenting a systems-level data model for durable, inspectable intermediate work products to improve AI work maintainability and revisability.


<details>
  <summary>Details</summary>
Motivation: Current agentic AI systems typically only retain final outputs and discard intermediate work, which is problematic for multi-step revisable AI tasks because final artifacts are often lossy projections of upstream state, lacking transparency and support for revision over time.

Method: The paper argues for preserving structured, typed, versioned, dependency-aware intermediate work products (distinct from private chain-of-thought, chat transcripts or memory), and proposes a formal systems-level data model with additive and superseding update semantics, explicit current-state resolution, and lineage tracking for intermediate artifacts.

Result: A complete systems-level data model for durable intermediate artifacts is defined, which clarifies the distinctions between intermediate artifacts and other related concepts (chain-of-thought, chat logs, memory, final outputs), and establishes that evaluation of agentic systems should include maintained-state quality in addition to final output quality.

Conclusion: Durable intermediate artifacts, rather than making models smarter, make AI-generated work significantly more inspectable, revisable, and maintainable over time, and should be treated as first-class citizens in agentic system design.
