...
### [1] [SlimPer: Make Personalization Model Slim and Smart](https://arxiv.org/abs/2607.12281)
...
TL;DR: 本文针对工业推荐系统中Transformer架构存在的内存和计算效率问题，提出了SlimPer个性化排序框架，该框架重新设计了个性化排序任务，降低了计算和内存开销，在Instagram Reels和Feed部署后提升了用户 engagement 并支持更长用户历史建模。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的架构在工业推荐系统中越来越流行，但Transformer的生成式自回归预设计要求保留随序列长度缩放的大尺寸中间张量，这与推荐任务仅需要为每个<用户,物品>对生成单个相关性分数、不需要词级监督的需求不匹配，存在计算和内存效率低下的问题，因此需要设计更适配推荐任务的个性化模型。

Method: SlimPer将个性化排序重新表述为紧凑统一的<用户,物品>知识库的迭代精炼过程：每层模型选择性查询原始多模态用户侧token，计算显式相关性匹配分数并精炼知识库，每层复杂度为O(N)且保持固定大小中间表示；将模型深度与用户历史长度解耦，还通过请求优化在所有候选物品间共享单份用户侧token拷贝进一步降低内存；该框架统一了稀疏、稠密和序列特征到单个骨干网络，通过注意力机制提供可解释性。

Result: 部署在Instagram Reels和Feed后，SlimPer实现了可衡量的用户engagement提升，精简了整体系统，并且能够有效对10k以上的细粒度用户历史事件进行建模。

Conclusion: 适配推荐任务特性设计的SlimPer框架，在保证模型对深度相关性的建模能力的同时，有效降低了计算和内存开销，在工业推荐场景中落地后取得了明确的性能和工程收益。
