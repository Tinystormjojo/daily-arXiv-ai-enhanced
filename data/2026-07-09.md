...
### [1] [Open-ended Multi-agent Autocurricula via Visual Inspection of Policies with Multi-modal LLMs](https://arxiv.org/abs/2607.08193)
*Lorenzo Pantè,Andrea Fanti,Roberto Capobianco*

Main category: cs.LG

TL;DR: 本文提出了一种利用多模态视频大语言模型（VLM）通过直接观察智能体策略的 episode 视频来生成开放式多智能体自课程的方法VIP，在SMAC多智能体任务上验证了其优于仅文本方法和基于标量任务得分的方法。


<details>
  <summary>Details</summary>
Motivation: 开放式强化学习课程设计的核心挑战是如何根据智能体当前的学习进度评估任务难度，以往工作仅使用标量任务得分或行为文本摘要，未能充分利用策略行为中直观的视觉信息。

Method: 提出Visual Inspection of Policies (VIP)方法，使用视频语言模型（文中用VideoLLaMa2-7B）直接处理记录的智能体交互episode视频，基于对策略行为的视觉观察评估任务难度，生成课程推荐，并将该方法拓展到多智能体场景，在StarCraft Multi-Agent Challenge (SMAC)上进行实验验证。

Result: 实验结果显示，即使使用轻量开源的VideoLLaMa2-7B，VIP生成的课程比仅文本消融版本和基于标量任务得分的方法都更有效，能训练出能力更强的通用智能体。

Conclusion: 利用多模态视频大语言模型直接从策略交互视频中评估任务难度、生成开放式自课程是一种有效可行的方案，效果优于传统的标量得分和纯文本方法，适用于多智能体场景。
...
TL;DR: 本文提出了名为DaV-Gen的端到端生成式检索框架，通过Draft-and-Verify（草稿-验证）机制解决传统多阶段级联架构优化目标不一致和现有端到端生成模型推理速度慢的问题，同时实现效率和精度提升。


<details>
  <summary>Details</summary>
Motivation: 主流工业信息检索（搜索和推荐）常用多阶段级联架构存在不同阶段优化目标不一致，会传播放大早期错误降低最终结果质量；而新兴端到端生成模型虽然能统一流水线，但仅解码器结构的自回归生成过程严重影响在线推理性能，因此需要提出新的框架解决上述问题。

Method: DaV-Gen受推测解码启发，将生成任务重设计为单个模型内两个协同操作：训练阶段使用复合损失函数同时优化候选草稿生成和细粒度验证，包括用于构建嵌入空间方便高效草稿生成的对比损失，以及融合生成似然和向量相似度得到验证分数的融合损失；推理阶段先通过高效基于向量的草稿生成得到候选集，再用能力更强的融合评分函数验证候选。

Result: 该框架在统一端到端架构内同时实现了稀疏草稿生成的速度优势和先进生成模型的精度优势。

Conclusion: DaV-Gen通过Draft-and-Verify机制有效解决了传统多阶段架构优化不一致和端到端生成模型推理效率低的问题，为搜索和推荐提供了一种新的统一范式。
