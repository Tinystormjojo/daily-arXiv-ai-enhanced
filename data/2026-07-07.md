...
### [1] [Responsible Personalisation: The Double-Edged Sword of Personalisation in Human-Robot Interaction](https://arxiv.org/abs/2607.06344)
...
TL;DR: 本文提出了一个基于生命周期、结合情境感知且考虑机器人具身性的负责任个性化人机交互（HRI）分析框架，对个性化HRI中的关键伦理风险进行了整合分析，并给出了可操作的设计建议和待解决的开放研究挑战，为系统化、符合伦理的个性化机器人行为研究奠定基础。


<details>
  <summary>Details</summary>
Motivation: 个性化已成为人机交互的核心能力，但现有关于负责任个性化的研究十分零散，缺乏对伦理风险如何在不同交互场景中产生的结构化理解；且HRI中机器人的具身性和社会存在会放大、重塑原有风险或产生新风险，这一缺口在HRI领域尤为关键。

Method: 提出一个基于生命周期、情境敏感且具备具身感知视角的框架，将个性化过程的不同阶段与交互特征（短期vs长期交互、开放域vs封闭域交互）结合，在此基础上对个性化HRI中的核心伦理风险进行整合分析，梳理不同场景下的风险表现，进而转化得到设计建议。

Result: 构建了可用于系统分析个性化HRI风险产生与演变过程的分析框架，梳理了自主性侵蚀、偏差用户建模、操纵、非人化、隐私侵犯等关键伦理风险在不同交互情境下的具体表现，得到了可指导实践的设计建议，同时明确了该领域的开放研究挑战。

Conclusion: 该工作结构化梳理了个性化HRI的设计空间与风险图景，为开发更系统化、透明化且符合伦理的个性化机器人行为方法提供了研究基础。
...
### [2] [Synthetic Consumer Insight Generation with Large Language Models](https://arxiv.org/abs/2607.05761)
*Stephen L. France,Pia. A. Albinsson*

Main category: cs.AI

TL;DR: This paper investigates whether large language models (LLMs) can generate high-quality synthetic consumer data for marketing projective techniques, comparing LLM outputs with real human responses across different settings and gives usage recommendations based on the findings.


<details>
  <summary>Details</summary>
Motivation: Modern data-driven marketing requires large amounts of consumer data, but collecting real consumer data is expensive, time-consuming, and hard to scale. There is a need to explore alternative low-cost scalable sources of consumer insight data.

Method: The authors test LLM-generated synthetic responses across multiple projective tasks, different LLMs, prompting strategies, and temperature settings. They compare these outputs with human participant responses from a primary study on city tourism destination perceptions, analyzing both sets of responses using linguistic measures, diversity/concentration metrics, topic models, and top-term analyses.

Result: There is substantial overlap between LLM and human responses in broad topics and consumer associations, but clear differences exist in writing style, linguistic structure, and how response diversity is generated. Response quality is shaped by model selection and prompting strategy choices.

Conclusion: LLMs can be used to generate useful synthetic consumer data for projective techniques when used appropriately, but users need to be aware of their limitations. The paper provides actionable recommendations for optimal utilization of LLMs for this use case.
