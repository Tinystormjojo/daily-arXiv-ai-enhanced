...
TL;DR: This paper compares two automated clinical report generation approaches (knowledge-engineered templates and zero-shot GPT-4) for remote cognitive remediation in low-resource settings with no reference reports, evaluates them by clinical experts, and provides design recommendations and a replicable evaluation methodology.


<details>
  <summary>Details</summary>
Motivation: Growing demand for cognitive remediation therapy with limited speech therapist availability has led to more remote rehabilitation tools, which generate large volumes of interaction data that clinicians cannot review efficiently. There is a need for automated clinical report generation in low-resource settings lacking reference reports, and no clear comparison between traditional rule-based and modern LLM-based approaches for this use case.

Method: Two approaches are compared on identical pre-extracted, expert-validated structured interaction data: 1) a rule-based template system encoding domain knowledge as explicit decision rules and validated templates; 2) a zero-shot GPT-4-based approach. Outputs are evaluated by 8 speech therapists and final-year students using a 9-criterion questionnaire.

Result: There is a clear trade-off between clinical reliability and linguistic quality: the template-based system scores higher on fluidity, coherence, and results presentation, while GPT-4 generates more concise output. Directional differences are consistent across evaluation dimensions, but no comparison reached statistical significance after correction due to the limited scale of expert clinical evaluation.

Conclusion: The work identifies a trade-off between traditional template-based and LLM-based approaches for clinical report generation, derives 8 design recommendations for clinical reporting systems in remote low-resource rehabilitation settings, and contributes a replicable methodology for controlled comparison of clinical natural language generation methods to inform responsible adoption of generative AI in healthcare.
...
TL;DR: 本文提出了面向工业广告场景中生成式推荐的统一价值对齐框架UniVA，在腾讯微信视频号广告平台的离线和在线实验中均取得显著效果提升。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐将推荐重构为下一个token生成问题在工业场景中展现出潜力，但将其拓展到工业广告领域时，系统需要同时优化用户兴趣和商业价值；而现有生成式推荐流程大多以语义为中心，难以在分词、解码和在线服务全流程中对齐价值信号。

Method: 提出统一价值对齐框架UniVA：1）设计商业SID分词器，将价值相关属性注入SID构建，得到具有价值区分度的物品表示；2）开发由监督学习和eCPM感知强化学习联合优化的"生成即排序"SID解码器，将价值分数融合到下一个物品SID生成中，在一次解码过程中同时完成生成和排序；3）设计价值引导的个性化束搜索，复用生成即排序的logits作为在线价值引导，并用个性化前缀树约束解码到请求有效的SID路径。

Result: 在腾讯微信视频号广告平台的实验中，相比基线模型离线Hit Rate@100提升37.04%，在线A/B测试中GMV提升1.5%。

Conclusion: UniVA框架可以有效解决工业广告生成式推荐中的价值对齐问题，在离线和在线场景均带来显著的效果提升，适合工业广告场景的落地应用。
...
TL;DR: 本文研究生成式推荐中自回归语义ID生成的表达能力限制，提出了一种简单的改进方法Latte，通过在每个语义ID前注入潜在令牌重构解码空间，提升了推荐性能。


<details>
  <summary>Details</summary>
Motivation: 生成式推荐模型通过自回归生成离散语义ID序列来索引物品，但其解码空间结构对模型表达能力的影响尚未得到充分研究；自回归逐 token 生成带来的解码树结构会使树中相近物品概率高度相关，难以根据用户偏好区分它们，且这种结构相关性甚至限制了模型表示简单模式的能力，因此需要解决该问题。

Method: 1. 首先将自回归token生成建模为语义ID令牌诱导的解码树遍历，分析了树结构带来的物品概率耦合问题，并从理论上证明了该结构会限制模型表达能力；2. 提出Latte方法，在每个语义ID前注入一个潜在令牌，将原来单一解码树重构为多个由潜在令牌条件化的解码树，通过不同树距离的多条路径缓解树诱导的概率耦合。

Result: 提出的Latte方法在NDCG@10指标上取得了平均3.45%的相对提升，验证了该改进方法的有效性；理论分析证明了原自回归生成结构无法表示传统协同过滤可以轻松捕获的简单模式。

Conclusion: 自回归语义ID生成的解码树结构会限制生成式推荐模型的表达能力，通过注入潜在令牌重构解码空间可以有效缓解该问题，显著提升推荐性能。
...
TL;DR: 本文针对对话搜索中对话开场白推荐存在的回声室效应、无法捕捉动态开放意图的问题，提出了PA-Bridge框架，利用用户主动输入的表达来改进推荐，在线A/B测试验证了其效果提升。


<details>
  <summary>Details</summary>
Motivation: 传统对话开场白推荐依赖封闭的"曝光-点击"反馈循环，结合数据稀疏性，会导致系统陷入回声室效应，无法捕捉开放世界中对话搜索意图的动态性，最终推荐偏向热门但通用的内容。而LLM驱动的对话搜索已经转向主动开放式对话，需要更有效的开场白推荐方法。

Method: 提出了PA-Bridge（Passive-Active Bridge）框架：1. 使用对抗分布对齐器弥合被动推荐的开场白和用户主动表达之间的分布差距；2. 引入语义离散器，使得大规模工业流训练可以应用流行度去偏算法，解决开放文本无法使用传统基于物品的流行度统计的问题。

Result: 在作者平台上的在线A/B测试显示，PA-Bridge显著将特征渗透率提升了0.54%，同时提升了用户活跃天数（摘要未给出完整数值）。

Conclusion: 利用用户主动输入的表达是打破传统对话开场白推荐封闭反馈循环的有效方向，PA-Bridge框架能够有效解决主动表达引入过程中的分布偏移和训练部署问题，带来显著的线上效果提升。
...
TL;DR: 本文针对人类移动性预测中不仅长尾POI、甚至热门POI预测也因转移级稀疏性失败的问题，提出了一个以转移为中心的组合泛化框架RECAP，利用全局转移图的多跳传递性和用户历史轨迹重访证据重构长尾转移，结合暖转移留出训练提升泛化能力，在多个真实数据集上验证了其对预测精度尤其是尾部转移预测的提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有研究关注了移动性预测中长尾POI（访问记录少）的预测难题，但本文分析发现即使是热门POI的预测也经常失败，根本原因在于训练集中源-目的地转移记录稀疏甚至缺失，即存在转移级长尾泛化瓶颈，因此需要解决这一核心问题提升移动性预测性能。

Method: 将转移级长尾泛化问题建模为组合泛化问题，提出RECAP框架：1）从两个可泛化信号重构长尾转移，分别是全局转移图中的多跳传递性、用户历史轨迹中的重访证据；2）采用暖转移留出训练策略，抑制模型对频繁转移的记忆，鼓励模型从可迁移信号学习泛化能力。

Result: 在多个真实世界数据集上的实验表明，RECAP能够持续提升人类移动性预测的准确率，对尾部转移的预测效果提升尤为显著。

Conclusion: 人类移动性预测的核心瓶颈是转移级长尾泛化，所提出的RECAP框架能够有效利用可泛化信号解决转移稀疏问题，显著提升预测性能尤其是尾部转移的预测精度。
...
TL;DR: 本文从软件研究批判视角出发，围绕当前人工智能发展和通用人工智能（AGI）提出五个核心问题，分析现有生成式AI的发展路径、不同基础模型发展轨迹的差异，并探索满足透明性、福祉等要求的通向AGI邻近能力的合理社会技术发展方案。


<details>
  <summary>Details</summary>
Motivation: 现有讨论常对当前AI发展现状做必然性假设，缺乏对当前商业AI发展与社会政治经济环境之间紧密关联的批判审视，同时AGI在概念和定义层面都存在问题，因此需要从批判视角梳理AGI发展路径并探索更合理的发展方向。

Method: 采用批判软件研究视角，提出五个关联问题展开分析：梳理当前主导生成式AI的形成路径，识别发展中的关键决策杠杆节点与失败的替代路径，对比不同类型基础模型（前沿专有模型、开放权重模型、领域/主权模型）的发展路径差异，分析分支替代项目的现状与成败原因，最终推导满足多元要求的合理AGI邻近能力发展方案。

Result: 抽象提出了分析现有AI发展路径与AGI未来方向的研究框架，明确了需要讨论的五个核心问题，指出当前AI发展路径受社会政治经济因素约束，AGI本身存在概念和定义层面的问题。

Conclusion: 需要以批判条件性视角审视AGI发展，梳理现有发展路径的关键节点与替代可能性后，可以推导出满足透明性、良性治理、福祉和可持续商业模式要求的通向AGI邻近能力的社会技术发展规划。
...
TL;DR: This paper proposes Temporal Smoothness Doubly Robust (TSDR) learning, a debiasing framework for Knowledge Tracing (KT) that handles selection bias in educational logs and mitigates variance accumulation over time, improving performance of existing state-of-the-art KT models on real-world benchmarks.


<details>
  <summary>Details</summary>
Motivation: Knowledge Tracing (KT) for intelligent education relies on selectively observed educational logs, which have severe selection bias from non-random exercise recommendations and student choices. Most existing KT methods ignore this bias, leading to biased mastery estimates and accumulated errors in downstream recommendations. Additionally, existing debiasing approaches for KT suffer from variance-dependent stochastic deviations that accumulate over sequential steps, causing training instability and limiting performance.

Method: First, a doubly robust (DR) formulation that integrates a propensity model and an error imputation model is introduced, which guarantees unbiasedness if either model is correct. Then, a generalization bound is derived to characterize the impact of estimator variance and identifies temporal smoothness as a key factor to control variance. Based on this, the TSDR framework is proposed, which jointly optimizes the KT predictor and imputation model with a temporal smoothness regularizer, preserving DR's unbiasedness guarantee while reducing variance.

Result: Experiments on multiple real-world KT benchmarks show that TSDR consistently improves performance of various state-of-the-art KT backbones, validating the effectiveness of the proposed debiasing and variance reduction approach.

Conclusion: Selection bias is an important issue that harms KT performance, and principled bias correction with temporal smoothness regularization can effectively mitigate this problem. The proposed TSDR framework preserves the unbiasedness guarantee of doubly robust learning while reducing accumulated variance over sequential steps, consistently enhancing existing state-of-the-art KT models.
