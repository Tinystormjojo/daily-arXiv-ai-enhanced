...
### [1] [MemDelta: Controlled Baselines and Hidden Confounds in Agent Memory Evaluation](https://arxiv.org/abs/2606.29914)
*Kuan Wang*

Main category: cs.CL

TL;DR: 本文提出了用于智能体记忆评估的控制评估协议MemDelta，指出当前智能体记忆系统评估中存在混淆变量的问题，通过控制单一变量变化进行实验得到四项关键发现，并给出了未来记忆评估的改进建议。


<details>
  <summary>Details</summary>
Motivation: 当前智能体记忆系统在与RAG和全上下文基线评估时， reported性能增益通常将记忆方法的变化与语言模型、嵌入模型或检索流水线的变化混杂在一起，导致无法明确测量的实际对象，评估结果存在隐藏混淆，因此需要可控的评估方案。

Method: 提出MemDelta可控评估协议，该协议每次仅改变一个组件，在LongMemEval-S数据集（包含500个问题、50多个会话、三个模型家族）上进行实验验证，通过统计对比分析不同组件变化对评估结果的影响。

Result: 得到四项核心发现：1.逐字RAG性能与全上下文GPT-4o-mini相当，但不同模型的排序不同，Gemini从全上下文中获益+14pp，Sonnet从RAG获益+31pp，部分因为Sonnet拒绝了63%的全上下文查询；2.仅更换相同流水线中的嵌入模型就能让准确率变化+6.2pp，单一变量就可以反转结论，Mem0比MiniLM-RAG高+11pp但比cloud-RAG低1.2pp；3.智能体自记忆（42%准确率）性能不如基础检索（47%）；4.在6种问题类型中的2种上，Mem0性能与cloud RAG相当（72.7% vs 73.9%），但成本是其50倍，仅存在窄收益而非通用收益。

Conclusion: 建议未来在进行记忆评估时，需要在不同比较中固定嵌入模型，按模型家族分层，并且在将收益归因于架构之前报告写入路径成本，以消除隐藏混淆获得可靠的评估结果。
...
### [2] [Exploration and Online Transfer with Behavioral Foundation Models](https://arxiv.org/abs/2606.29980)
*Louis Bagot,Mathieu Lefort,Laëtitia Matignon*

Main category: cs.LG

TL;DR: 本文研究行为基础模型（BFM）在零样本强化学习中的在线迁移问题，提出利用BFM本身生成探索策略，将问题建模为类老虎机的探索-利用问题，在线性奖励近似下推导了基于置信上界的方法，并在简单环境中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本迁移强化学习的行为基础模型框架，在迁移阶段假设智能体可以通过状态-奖励对数据集离线获知奖励，但实际中奖励常为黑盒（如直接用户反馈），无法生成这类数据集，需要通过与环境交互观察奖励，现有离线迁移框架不匹配需要探索的在线试错学习设定。

Method: 提出解决零样本强化学习的在线迁移新问题，利用行为基础模型本身生成探索策略，将在线学习问题建模为类老虎机的探索-利用问题：每一步老虎机算法推荐策略，BFM在环境中执行得到奖励和新状态，重复直到收敛到最优策略；在线性奖励近似下，推导了受置信上界启发的公式，证明可以通过最小化不确定性矩阵的特征值实现探索。

Result: 在简单环境上进行了定性和定量评估，验证了所提方法概念的有效性。

Conclusion: 所提框架能够解决奖励黑盒场景下行为基础模型的在线迁移探索问题，利用BFM自身生成探索策略的思路有效，在线性奖励近似下的方法可行，通过实验验证了该概念的合理性。
